...
var invokeChaincode = function (peers, channelName, chaincodeName, fcn, args, org_name) {
	...
	return helper.getClientForOrg(org_name).then(_client => {
		...
		if (all_good) {
			...
			return Promise.all([sendPromise].concat(eventPromises))
				.then((results) => {
					...
					return Promise.resolve(results[0]); // the first returned value is from the 'sendPromise' which is from the 'sendTransaction()' call
				}).catch((err) => {
		...
		if (response.status === 'SUCCESS') {
			...
			return Promise.resolve({ success: true, message: 'Successfully invoke transaction' });
		} else {
