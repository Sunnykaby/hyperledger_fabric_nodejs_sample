...
var instantiateChaincode = function (peers, channelName, chaincodeName, chaincodeVersion, functionName,
	...
	return helper.getClientForOrg(org_name).then(_client => {
		...
		logger.error({ role: { name: 'admin', mspId: ORGS[org_name].mspid } });
		...
		if (all_good) {
			...
			return Promise.all([sendPromise].concat(eventPromises))
				.then((results) => {
					...
					return Promise.resolve(results[0]); // just first results are from orderer, the rest are from the peer events
		...
		if (!(response instanceof Error) && response.status === 'SUCCESS') {
			...
			return Promise.resolve({success:true, message:"Successfully Instantiate the chaincode"});
		} else {
			return Promise.reject(new Error('Failed to order the ' + type + 'transaction. Error code: ' + response.status));
		}
	}, (err) => {
		return Promise.reject(new Error('Failed to send instantiate due to error: ' + err.stack ? err.stack : err));
	});
};
